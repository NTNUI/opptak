...
function ApplicationPeriod() {
    ...
    const [previousDates, setPreviousDates] = useState<(Date|null)[]>([null,null])
    ...
    const form = useForm({
        ...
        validate: {
            datePeriod: value => !value[0] || !value[1] ? 'Du må velge en tidsperiode' : null
        }
    })
	...
	return (
		<div className={classes.pageWrapper}>
			...
			/>
            {!changed &&  <i className={classes.unchangedText}>Opptaksperioden er uendret</i>}
			<div className={classes.buttonWrapper}>
			    <Button color="red" leftIcon={<X/>}>Avbryt</Button>
    			<Button color="green" disabled={!changed} leftIcon={<Check/>}>Lagre</Button>
			</div>
		</div>
	)
}
